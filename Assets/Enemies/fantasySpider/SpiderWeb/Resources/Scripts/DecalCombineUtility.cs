...
/*
 Slightly modified version of MeshCombineUtility. Renamed to prevent conflicts.
*/

public class DecalCombineUtility {
		...
		foreach( MeshInstance combine in combines )
		{
			if (combine.mesh)
			{
				...
				if (generateStrips)
				{
					...
					int curStripCount = combine.mesh.GetTriangles(combine.subMeshIndex).Length; //phil: I found on the internet to change GettriangleStrips to GetTriangles and it removed the error.
					if (curStripCount != 0)
					{
						if( stripCount != 0 )
						{
