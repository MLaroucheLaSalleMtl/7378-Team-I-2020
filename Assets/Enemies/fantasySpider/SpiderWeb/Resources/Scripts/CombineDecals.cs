...
COmbienChildren script. Slightly modified for SMD. Renamed to prevent conflicts.
*/

public class CombineDecals : MonoBehaviour {
public bool on = false;
	...
	void LateUpdate () {
		...
		for (int i=0;i<filters.Length;i++) {
			...
			GameObject curObj = filters[i].gameObject;
			DecalCombineUtility.MeshInstance instance = new DecalCombineUtility.MeshInstance ();
		...
		foreach (DictionaryEntry de  in materialToMesh) {
			...
			DecalCombineUtility.MeshInstance[] instances = (DecalCombineUtility.MeshInstance[])elements.ToArray(typeof(DecalCombineUtility.MeshInstance));
			...
			if (materialToMesh.Count == 1)
			{
				...
				filter.mesh = DecalCombineUtility.Combine(instances, generateTriangleStrips);
